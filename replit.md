...
- **Monitor Dashboard Layout**: Two-column grid layout with left column (lg:col-span-8) containing Insight Cards, Warranty Expiration Heatmap, Critical Risk Models section, and Critical Models Without Coverage section; right column (lg:col-span-4) containing KPI Cards (Critical/High Risk counts) and Low Coverage Pools section. **Critical Risk Models** displays up to 30 critical-priority models (run_rate >= 1 AND days_of_supply < 30) in a responsive 3-column card grid with red left borders, showing models running out soonest, includes both models with and without active warranty coverage. **Critical Models Without Coverage** is a separate section below Critical Risk Models, displaying up to 20 critical models that ALSO have zero active warranty coverage (covered_count = 0 AND run_rate >= 1 AND days < 30) in a 3-column grid with amber left borders, sorted by days of supply. Both sections feature identical card layouts (model name with processor/generation, days remaining badge, coverage status indicator, 5-column metrics row for Active/Spare/Run/UK/UAE counts, bulk selection checkboxes, and quick-action Alert/Pool buttons), maintain separate bulk-selection state for independent actions, and include "View All" links to the Risk Combinations page with appropriate filters. Cards use minimal spacing and compact typography for maximum information density.
...
- **Model Statistics API**: `GET /api/models/stats` endpoint returns **procurement-focused statistics for models requiring immediate attention** (run_rate >= 1.0 AND daysOfSupply <= 60). Provides actionable data for purchase order generation including: covered units count, spare units count, regional stock distribution (UK/UAE), run rate (average claims per month from last 6 months), spare rate (percentage of spare units to monthly run rate), days of supply (until spare stock depleted), coverage ratio (spare units to covered units percentage), **unitsNeededFor2Months** (exact procurement quantity to achieve 60-day coverage), and risk level categorization (critical/high only). Filters out low-demand models (run_rate < 1.0) to focus on actionable procurement opportunities. Supports sorting by make, model, runRate, spareRate, daysOfSupply, coveredCount, spareCount, or coverageRatio. Uses server-side aggregation with CTEs for efficient calculation. Designed for procurement planning, purchase order workflows, and urgent inventory replenishment without pagination constraints.
