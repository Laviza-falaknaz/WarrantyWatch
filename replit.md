...
- **UI Features**: Monitor Dashboard includes compact card view with advanced filtering (search by make/model/processor, risk level dropdown), toggle to include/exclude 0-covered units (default: exclude), checkbox-based multi-selection, bulk actions (Send Combined Alert, Create Combined Pool) that appear when items selected, individual action buttons (Send Alert with Bell icon, Create Pool with FolderPlus icon), and View All Risk Profiles link. Full table view with 8 columns available on dedicated page with additional filtering capabilities.
