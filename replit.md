...
- **Warranty Explorer**: Comprehensive warranty management interface connected to the `covered_unit` database table. Features include: (1) Summary cards for Total Warranties, Active Coverage, Expiring Soon, and Customers metrics; (2) Advanced filtering with case-insensitive multi-select filters for Make, Model, Customer, Order Number, Processor, RAM, Category, and Coverage Type; (3) Status filter (Active/Expired) based on `isCoverageActive` column; (4) Global search across serial number, make, model, customer, and order; (5) Data table with warranty details including make/model, specifications, customer info, coverage description, and expiration dates; (6) Color-coded status badges (Active, Expires in Xd, Expired); (7) Pagination (50 records per page); (8) Excel export functionality. All filters use React Query with proper query param passing, and filter options are loaded from entire dataset to support pagination.
