...
- **Model Statistics API**: `GET /api/models/stats` endpoint returns comprehensive statistics for all unique model combinations (make, model, processor, generation). Provides complete overview including: covered units count, spare units count, regional stock distribution (UK/UAE), run rate (average claims per month from last 6 months), spare rate (percentage of spare units to monthly run rate), days of supply (until spare stock depleted), coverage ratio (spare units to covered units percentage), and risk level categorization. Supports sorting by make, model, runRate, spareRate, daysOfSupply, coveredCount, or spareCount. Uses server-side aggregation with CTEs for efficient calculation across all inventory tables. Designed for reporting, analytics, and inventory planning without pagination constraints.
