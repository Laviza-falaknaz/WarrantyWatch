...
- **Units Running Out - Two-Part Architecture**: (1) **Monitor Dashboard Summary**: Displays top 10 models running out soonest, sorted by days of supply remaining. Shows KPI cards (Critical/High risk counts), ranked list with days remaining prominently displayed, active warranty count badges, regional stock splits (UK/UAE), and quick-action buttons (Pool/Alert) per model; (2) **Risk Combinations Page** (/risk-combinations): Full-featured analysis page with server-side pagination, sortable table (default: days of supply ASC), comprehensive filtering (coverage % range, risk levels, make/model, search), covered/not-covered categorization, bulk actions, and CSV export. Both views share the same `/api/risk-combinations` backend API with different query parameters. **Risk Categorization**: Based on days of supply remaining - Critical (<30 days), High (30-60 days), Medium (60-120 days), Low (120+ days). Calculation: days_of_supply = (spare_count / run_rate_per_month) * 30.
...
- **Units Running Out Analysis System**: `GET /api/risk-combinations` endpoint with multi-tier risk scoring (Critical, High, Medium, Low) based on days of supply remaining. **Risk Thresholds**: Critical (<30 days), High (30-60 days), Medium (60-120 days), Low (120+ days). **Calculation**: days_of_supply = (spare_count / run_rate_per_month) * 30. Supports server-side pagination, sorting (by days_of_supply, coverage ratio, risk score, spare count, run rate - default: daysOfSupply ASC with NULLS LAST), status filtering (`?status=active|inactive|all`) for warranty lifecycle management, and advanced filtering (coverage % range, risk levels, make/model, search). Returns `{ data, total, stats }` format where stats are calculated from entire filtered dataset using SQL CTEs before pagination, ensuring accurate critical/high/medium/low counts and worst deficit across all pages. Used by both Monitor Dashboard (top 10 summary with `limit=10&sortBy=daysOfSupply&sortOrder=asc&excludeZeroCovered=true`) and Risk Combinations page (full paginated view with user-controlled filters/sorting). Dedicated `/api/risk-combinations/export` endpoint provides truly unpaginated CSV export of complete filtered dataset with formula injection protection (sanitizes =+-@ prefixes).
