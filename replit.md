...
- **Monitor Dashboard Layout**: Two-column grid layout with left column (lg:col-span-8) containing Insight Cards, Warranty Expiration Heatmap, and Models Needing Attention card; right column (lg:col-span-4) containing KPI Cards (Critical/High Risk counts) and Low Coverage Pools section. Models Needing Attention displays top 10 models running out soonest, sorted by days of supply remaining, with days remaining prominently displayed, active warranty count badges, regional stock splits (UK/UAE), and quick-action buttons (Pool/Alert) per model.
