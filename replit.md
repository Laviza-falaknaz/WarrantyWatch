...
- **Units Running Out - Two-Part Architecture**: (1) **Monitor Dashboard Summary**: Displays top 5-10 worst-performing models with KPI cards (Critical/High risk counts), ranked list with progress bars showing coverage %, and quick-action buttons (Pool/Alert) per model; (2) **Risk Combinations Page** (/risk-combinations): Full-featured analysis page with server-side pagination, sortable table, comprehensive filtering (coverage % range, risk levels, make/model, search), covered/not-covered categorization, bulk actions, and CSV export. Both views share the same `/api/risk-combinations` backend API with different query parameters.
...
- **Units Running Out Analysis System**: `GET /api/risk-combinations` endpoint with multi-tier risk scoring (Critical, High, Medium, Low) based on spare count and coverage ratio, plus status filtering (`?status=active|inactive|all`) for warranty lifecycle management. Supports server-side pagination, sorting (by coverage ratio, risk score, spare count, run rate), and advanced filtering (coverage % range, risk levels, make/model, search). Returns `{ data, total, stats }` format where stats are calculated from entire filtered dataset using SQL CTEs before pagination, ensuring accurate critical/high/medium/low counts and worst deficit across all pages. Used by both Monitor Dashboard (top 10 summary with `limit=10&sortBy=coverage_of_run_rate&sortOrder=asc&excludeZeroCovered=true`) and Risk Combinations page (full paginated view with user-controlled filters/sorting). Dedicated `/api/risk-combinations/export` endpoint provides truly unpaginated CSV export of complete filtered dataset with formula injection protection (sanitizes =+-@ prefixes).
