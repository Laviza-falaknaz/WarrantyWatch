...
- **Monitor Dashboard Layout**: Two-column grid layout with left column (lg:col-span-8) containing Insight Cards, Warranty Expiration Heatmap, and Models Without Coverage section; right column (lg:col-span-4) containing KPI Cards (Critical/High Risk counts) and Low Coverage Pools section. Models Without Coverage displays up to 30 models with **zero active warranty coverage** (covered_count = 0) in a responsive 3-column card grid (1 col mobile, 2 cols tablet, 3 cols desktop) showing models sorted by days of supply remaining. This includes both critical high-demand models (run_rate >= 1) and low-demand models (run_rate < 1) that have claims but no coverage. Each compact card features: risk-level colored left border, model name with processor/generation in title, days remaining badge, "Has Active Warranties"/"No Active Warranties" indicator, 5-column metrics row (Active, Spare, Run/mo, UK, UAE counts), bulk selection checkboxes, and quick-action buttons (Alert/Pool). Cards use minimal spacing and compact typography for maximum information density.
