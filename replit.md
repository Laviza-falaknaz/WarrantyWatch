...
- **Monitor Dashboard Layout**: Two-column grid layout with left column (lg:col-span-8) containing Insight Cards, Warranty Expiration Heatmap, and Critical Risk Models section; right column (lg:col-span-4) containing KPI Cards (Critical/High Risk counts) and Low Coverage Pools section. Critical Risk Models displays up to 30 **critical-only** models (won't last a month) in a responsive 3-column card grid (1 col mobile, 2 cols tablet, 3 cols desktop) showing models running out soonest, sorted by days of supply remaining. Each compact card features: red left border indicating critical status, model name with processor/generation in title, days remaining badge, "Has Active Warranties"/"No Active Warranties" indicator, 5-column metrics row (Active, Spare, Run/mo, UK, UAE counts), bulk selection checkboxes, and quick-action buttons (Alert/Pool). Cards use minimal spacing and compact typography for maximum information density.
