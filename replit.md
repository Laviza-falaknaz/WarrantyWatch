...
- **Key Pages** (All Redesigned November 2025):
  ...
  - **Risk Combinations** (`/risk-combinations`) - Dedicated full-page data table for comprehensive risk analysis with 4 stat cards (Total Combinations, Critical Risk, High Risk, Selected), advanced filtering (search by make/model/processor, risk level dropdown, toggle to exclude 0-covered units), sortable table columns (Risk Score, Spare Count, Monthly Run Rate, Coverage Ratio, Fulfillment Rate), checkbox-based multi-selection with page-level Select All/Deselect All, pagination (50 items per page with automatic page reset on filter changes), individual row actions (Send Alert, Create Pool), bulk actions (Send Combined Alert, Create Combined Pool) that appear when items selected, real API integration for pool creation and alert sending with comprehensive error handling and toast notifications
...
- **Endpoint**: `GET /api/risk-combinations` with server-side filtering for security and scalability, supports query parameters for sorting (sortBy, sortOrder), pagination (limit, offset), and filtering (excludeZeroCovered).
...
- **UI Features**: 
  - **Monitor Dashboard**: Compact card view (shows top 10) with advanced filtering (search by make/model/processor, risk level dropdown), toggle to include/exclude 0-covered units (default: exclude), stable selection system using composite keys (make|model|processor|generation) for persistence across filter changes, checkbox-based multi-selection with intelligent Select All/Deselect All buttons, bulk actions (Send Combined Alert, Create Combined Pool) that appear when items selected, individual action buttons (Send Alert with Bell icon, Create Pool with FolderPlus icon) that hide when multiple items selected, and "View All Risk Profiles" button navigating to dedicated page.
  - **Risk Combinations Page**: Full-page data table with sortable columns, client-side search and filtering, pagination (50 items/page with automatic page reset), multi-selection with Select All/Deselect All (operates on current page), individual and bulk actions with real API integration.
- **Actions**: Single-item and bulk actions for sending alerts (POST /api/risk-combinations/send-alert) and creating pools (POST /api/coverage-pools) with comprehensive error handling, toast feedback, cache invalidation, and selection clearing on success.
- **Alert Webhook**: Configured in Configuration page under Integrations tab, sends structured JSON payloads to Power Automate webhook URL.
